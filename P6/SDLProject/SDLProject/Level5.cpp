#include "Level5.h"
#define LEVEL5_WIDTH 14
#define LEVEL5_HEIGHT 8
#define LEVEL5_ENEMY_COUNT 6
#define LEVEL5_CAT_COUNT 4
GLuint fontTextureID5;
glm::vec3 randomvec15 = glm::vec3(1, -1, 0);
glm::vec3 randomvec55 = glm::vec3(5, -1, 0);
...
void Level5::Initialize() {
    ...
    state.cat = new Entity[LEVEL5_CAT_COUNT];
    GLuint catTextureID = Util::LoadTexture("cat.png");
    for (int i = 0; i < LEVEL5_CAT_COUNT; i++){
...
int Level5::Update(float deltaTime, int playerLives) {
    state.player->Update(deltaTime, state.player, state.enemies, LEVEL5_ENEMY_COUNT, state.map);
    for(int i = 0; i < LEVEL5_ENEMY_COUNT; i++){
        state.enemies[i].Update(deltaTime, state.player, state.enemies, LEVEL5_ENEMY_COUNT, state.map);
    }
    for(int i = 0; i < LEVEL5_CAT_COUNT; i++){
        state.cat[i].Update(deltaTime, state.player, state.enemies, LEVEL5_ENEMY_COUNT, state.map);
    }
    ...
    if (state.cat[0].isActive == false && state.cat[1].isActive == false && state.cat[2].isActive == false && state.cat[3].isActive == false && playerLives > 0 && state.player->position.x > 11.0f && state.player->position.y<-6.0f) {
        state.nextScene = 6;
    }
    return playerLives;
}
void Level5::Render(ShaderProgram *program, int playerLives) {
    ...
    if (playerLives == 1){
        Util::DrawText(program, fontTextureID5 , "Lives : 1", 0.8, -0.5f, randomvec15);
    }
    if (playerLives == 2){
        Util::DrawText(program, fontTextureID5 , "Lives : 2", 0.8, -0.5f, randomvec15);
    }
    if (playerLives == 3){
        Util::DrawText(program, fontTextureID5 , "Lives : 3", 0.8, -0.5f, randomvec15);
    }
    if (playerLives == 0){
        Util::DrawText(program, fontTextureID5 , "you lost :(", 0.8, -0.5f, randomvec55);
    }
