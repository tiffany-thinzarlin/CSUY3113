...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Save Da Cat!", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
    ...
    music = Mix_LoadMUS("Mister-Sneakypants.mp3");
    Mix_PlayMusic(music, -1);
    Mix_VolumeMusic(MIX_MAX_VOLUME / 4);
    
    meow = Mix_LoadWAV("meow.wav");
    Mix_VolumeChunk(meow, MIX_MAX_VOLUME / 10);
...
void Update() {
    if (currentSceneIndex != 0 && playerLives > 0){
        ...
        viewMatrix = glm::mat4(1.0f);
        if (currentScene->state.player->position.x >8){
